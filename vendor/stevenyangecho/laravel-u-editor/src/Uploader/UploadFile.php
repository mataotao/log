    ...
    public function doUpload()
    {
        ...
        if(config('UEditorUpload.core.mode')=='local'){
            try {
                ...
                //同步一份到图片服务器
                $dir_path = ltrim(dirname($this->filePath),'/');
                $dir_arr = explode('/',$dir_path);
                for($i=0;$i<count($dir_arr);$i++){
                    $mkdir_path = $dir_arr[0];
                    if($i > 0){
                        for($j=1;$j<=$i;$j++){
                            $mkdir_path.= '/'.$dir_arr[$j];
                        }
                    }
                    Ftp::connection('connection1')->makeDir($mkdir_path);
                }

                $uploadFile = Ftp::connection('connection1')->uploadFile(dirname($this->filePath).'/'.$this->fileName,$dir_path.'/'.$this->fileName,FTP_ASCII);//or FTP_BINARY
