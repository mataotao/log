    ...
    $(function(){
        function  choice_from(){
            ...
            $(".edit_role").click(function(){
                $('#Form2').find('.mybox').show();
                $('#Form2').find('input[name=logo]').val($(this).attr('data-logo'));
                $('#Form2').find('.edit1').attr('src',$(this).attr('data-url')+$(this).attr('data-logo'));
                ...
                $("#edit_sort").val($(this).parent().siblings(".role_sort").text());
                $("#edit_summary").val($(this).parent().siblings(".role_summary").text());

                $('#Form2').find('input').attr('disabled',false);
                $('#Form2').find('textarea').attr('disabled',false);
                $('#Form2').find('#sure-notice').show();
                $("#Form2").show();
                $("#Form1").hide();
            })
            ...
            $(".check_role").click(function(){
                ...
                $('#Form2').find('input').attr('disabled',true);
                $('#Form2').find('textarea').attr('disabled',true);
                $('#Form2').find('#sure-notice').hide();
                $("#Form2").show();
                $("#Form1").hide();
            })
        }
        ...
        function frozen_curriculum(){
            $('.frozen_curriculum').click(function(){
                var id = $(this).attr('data');
                var status = $(this).attr('data1');
                if(status == 1){
                    tishiyu = '操作后用户将无法访问此内容，确认进行此操作？！';
                }else{
                    tishiyu = '操作后用户能够再次访问内容，确认进行此操作？！';
                }
                var delete_url = $(this).attr('url');
                layer.confirm(tishiyu, {
                    title:'删除',
                    btn: ['是','否'] //��ť
                }, function(){
                    window.location.href=delete_url+'&status='+status+'&id='+id;
                });
            });
        }
        frozen_curriculum();
