    ...
    $(function(){
        ...
        $('#Form2').delegate('#sure-notice','click', function () {
           var action = $('#Form2').attr('action');
            $.post(action,{service_price:$('#Form2').find('input[name=service_price]').val()}, function (res) {
                obj = eval('(' + res + ')');
                if(obj.code == 1){
                    $('#yiyouhui_jiage').html('月报需求：¥'+$('#Form2').find('input[name=service_price]').val()+'元/月');
                    $('.yiyouhui_jiage').html('¥'+$('#Form2').find('input[name=service_price]').val()+' &nbsp;元/月');
                }else {
                    layer.msg(obj.msg)
                }
            })
